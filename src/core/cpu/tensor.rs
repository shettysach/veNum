    ...
    T: Copy,
{
    ...
    pub fn reshape(&self, sizes: &[usize]) -> Tensor<T> {
        Tensor::init(&self.data, sizes, self.shape.offset)
    }
    ...
    pub fn index(&self, index: &[usize]) -> T {
        self.data
            .get(self.shape.index_to_position(index))
            .copied()
            .unwrap()
    }
    ...
    fn broadcasted_shapes_map<R>(&self, rhs: &Tensor<T>, f: impl Fn(T, T) -> R) -> Tensor<R> {
        let broadcasted_sizes = self.shape.broadcast(&rhs.shape);
        let broadcasted_shape = Shape::new(&broadcasted_sizes, 0);

        let lhs_broadcasted = self.expand(&broadcasted_sizes);
        let rhs_broadcasted = rhs.expand(&broadcasted_sizes);

        let result = IndexIterator::new(&broadcasted_shape)
            .map(|index| {
